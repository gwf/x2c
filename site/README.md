...
The X Lisp and X macros cards count every tracked `.xlisp` and `.xmacro`
file in the repository, including tests, examples, and packages, skipping
files that begin with a `// Generated by` header, such as the Lisp
`tools/gen-lisp-init.py` writes. Embedded Lisp
