/*  fill.x -- filling an int buffer with a chosen value.

    The same four proof steps clear.x uses, with the ghost value `v__pre`
    where clear.x wrote the literal `0i`. Nothing in the proof library
    changed to accept it: the cursor theory is stated over an arbitrary
    element, and the invariant carries the extra parameter's cell.
*/
