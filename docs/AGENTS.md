...
`docs/src/library/modules/` and the `# Libraries and Packages` section of
`docs/src/SUMMARY.md` are generated by `tools/gen-api-reference` from what
`builds/0/x2c translate --dump-definitions` reports for `lib/`. Never
hand-edit either; run `make doc-generate`.
...
`docs/src/library/` are authored; only `modules/` is generated there.

The generator's `--check-path <path>` mode audits compiler API, optional-library
API, and public-type surfaces without adding pages to the book. It reads the
same compiler projection, so signatures and names are the compiler's own.
