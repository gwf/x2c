...
| `String.contains_digit` | lib/string-classify.x / String value operations | implementable with current values | Reports whether `str` contains a decimal digit. A null or empty `String` returns false. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.contains_digit to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.count` | lib/string.x / String value operations | implementable with current values | Returns the non-overlapping count of `sub` in `str`. A null or empty `str` or `sub` returns zero. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.count to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.dedent` | lib/string.x / String value operations | implementable with current values | Removes the indentation the text was written with. The prefix is the run of spaces and tabs that opens the first content line, after one leading newline is dropped. Every following line that starts with that prefix loses exactly it, so indentation written past the prefix survives and the block renormalizes as a unit. A line that does not carry the prefix, including a blank one, is left alone, except that a final line of only spaces and tabs is removed so the closing quote's own indentation does not reach the result. `\r\n` is preserved because only the prefix after a newline is removed. Raises: `<alloc-fail>` while constructing a changed result. Null input returns NULL and text with no prefix is returned as-is. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.dedent to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.endswith` | lib/string.x / String value operations | implementable with current values | Reports whether `str` ends with `suffix`. An empty `suffix` is a suffix of every `String`. The comparison is bytewise, so this is a safe test for a file extension but not for a case-insensitive one; lower both sides first. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.endswith to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.equal` | lib/string.x / String value operations | implementable with current values | Reports whether `x` and `y` contain the same bytes. Interning already makes `x == y` a content test for two canonical `String`s, so use this when one side may be a transient `String.malloc` buffer or when null has to compare cleanly. Two nulls are equal, since the null pointer is the empty `String`, and a null equals no non-empty `String`. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.equal to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.escape` | lib/string.x / String value operations | implementable with current values | Returns a canonical escaped representation of the bytes in `str`. Common control and delimiter bytes use named escapes, printable ASCII is copied, and every other byte uses a three-digit octal escape. `Null` input or an oversized result returns NULL. Raises: `<alloc-fail>` while constructing the result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.escape to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.filter` | lib/string.x / String interpreted callbacks | needs callback adapter | Returns the bytes of `str` accepted by ordinary `Var` truthiness of `fn`. Each byte is boxed from `char` and passed by value. A null or empty `str`, or a null `fn`, returns `str` without invoking the callback. Otherwise `fn` is called once per byte from left to right and is not retained. Raises: whatever `Func.apply`, `fn`, or the returned `Var`'s truth operation raises, or `<alloc-fail>` when the result cannot be allocated. | The runtime takes native Func while meta closures are evaluator callables. Use Lisp.apply through a reusable Func context for String.filter; check its documented callback order, null callback and empty-input cases. |
| `String.find` | lib/string.x / String value operations | implementable with current values | Returns the index of the first occurrence of `sub` in `str`, or -1. The search is byte-oriented rather than character-oriented, so an index may land inside a multibyte sequence. An empty `sub` matches at index 0. Use `String.find_within` to bound the search to a range, or `String.rfind` to scan from the end. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.find to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.find_all` | lib/string.x / String value operations | implementable with current values | Returns each non-overlapping starting index at which `sub` occurs. The search begins at `start` and uses the same normalized exclusive `end` as `String.find_within`. `Null` or empty `str` or `sub` returns `nil`. The result is a canonical `List` whose cells follow their owning pools. Raises: `<alloc-fail>` or `<size-limit>` while constructing the result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.find_all to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.find_within` | lib/string.x / String value operations | implementable with current values | Returns the first index of `sub` within `str[start:end]`, or -1. The returned index is absolute, measured from the start of `str` rather than from `start`. Negative `start` and `end` count from the end of `str`, and both are then clamped to the `String`. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.find_within to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
...
| `String.is_alnum` | lib/string-classify.x / String value operations | implementable with current values | Reports whether every byte of nonempty `s` is alphanumeric. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.is_alnum to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.is_alnum_under` | lib/string-classify.x / String value operations | implementable with current values | Reports whether nonempty `s` is alphanumeric or underscore. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.is_alnum_under to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.is_alpha` | lib/string-classify.x / String value operations | implementable with current values | Reports whether every byte of nonempty `s` is alphabetic. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.is_alpha to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.is_alpha_under` | lib/string-classify.x / String value operations | implementable with current values | Reports whether every byte of nonempty `s` is alphabetic or underscore. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.is_alpha_under to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.is_digit` | lib/string-classify.x / String value operations | implementable with current values | Reports whether every byte of nonempty `s` is a decimal digit. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.is_digit to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.is_identifier` | lib/string-classify.x / String value operations | implementable with current values | Reports whether `s` is a C identifier under C character classes. The first byte must be alphabetic or underscore; later bytes may also be decimal digits. A null or empty `String` returns false. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.is_identifier to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.is_lower` | lib/string-classify.x / String value operations | implementable with current values | Reports whether every byte of nonempty `s` is lower-case. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.is_lower to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.is_lower_under` | lib/string-classify.x / String value operations | implementable with current values | Reports whether every byte of nonempty `s` is lower-case or underscore. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.is_lower_under to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
...
| `String.pad_right` | lib/string.x / String value operations | implementable with current values | Pads the right side of `str` to the requested width. Raises: the same causes as `String.pad_left`. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.pad_right to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
...
| `String.remove_suffix` | lib/string.x / String value operations | implementable with current values | Removes `suffix` when `str` ends with it and returns a canonical `String`. A null or absent suffix returns `str` unchanged; removing the complete `String` returns NULL. Raises: `<alloc-fail>` while constructing a changed result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.remove_suffix to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.repeat` | lib/string.x / String value operations | implementable with current values | Returns a canonical `String` containing `count` copies of `str`. `Null` or empty input, a nonpositive count, or an oversized result returns NULL. A count of one returns `str` when it is already visible in the active pool; a transient buffer is copied and interned instead. Raises: `<alloc-fail>` while constructing a nonempty result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.repeat to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.replace` | lib/string.x / String value operations | implementable with current values | Returns `str` with every occurrence of `old` replaced by `replacement`. Scanning runs left to right and matches do not overlap: each one resumes after the text it consumed, so replacing `aa` in `aaaa` performs two replacements, not three. Replaced text is never rescanned, so a `replacement` that contains `old` does not loop. An empty `replacement` deletes the matches. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.replace to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.replace_n` | lib/string.x / String value operations | implementable with current values | Replaces at most `max_replacements` non-overlapping occurrences of `old`. Scanning proceeds left to right and does not rescan replacement text. A negative limit replaces all matches; zero, null or empty `old`, null input, or no match returns `str` unchanged. `Null` `replacement` deletes matches. An oversized result returns NULL. Raises: `<alloc-fail>` while constructing a changed result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.replace_n to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
...
| `String.unescape` | lib/string.x / String value operations | implementable with current values | Decodes supported backslash escapes in `str` into a canonical `String`. Standard single-byte escapes, up to two hexadecimal digits after `x`, `u`, or `U`, and up to three octal digits are consumed. A backslash-newline is removed, an unknown escape yields its following byte, and a trailing backslash is dropped. `Null` input returns NULL and input without a backslash is returned unchanged. Raises: `<bad-arg>` for an octal escape above `\377`, which does not fit a byte, or `<alloc-fail>` while constructing a changed result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.unescape to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.upper` | lib/string.x / String value operations | implementable with current values | Returns `str` with every lower-case byte raised. Like `String.lower`, mapping runs byte by byte through C's `toupper` and covers ASCII in the default locale. `str` itself is returned when nothing would change. Raises: `<alloc-fail>` while constructing the result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.upper to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.var` | lib/common.x / String value operations | implementable with current values | Boxes a `String` value as `Var`. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.var to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `String.withindex` | lib/string.x / String value operations | implementable with current values | Returns a canonical copy of `str` with the byte at `index` set. Canonical `String`s are immutable, so `str` is not modified. When the byte already has `value`, this may return `str`; otherwise a new `String` is built and interned. Native assignment `str[index] = value` writes through shared canonical storage and belongs only on a transient `String.malloc` buffer. A negative `index` counts from the end. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind String.withindex to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
...
| Callable | Implementation owner / group | Disposition | Runtime contract | Contract and next action |
| --- | --- | --- | --- | --- |
| `List.all` | lib/list.x / List interpreted callbacks | needs callback adapter | True when every element satisfies `pred` by ordinary `Var` truthiness. Stops at the first rejection. Nil is true; a null `pred` is false for a nonempty `List`. | The runtime takes native Func while meta closures are evaluator callables. Use Lisp.apply through a reusable Func context for List.all; check its documented callback order, null callback and empty-input cases. |
| `List.any` | lib/list.x / List interpreted callbacks | needs callback adapter | True when at least one element satisfies `pred` by ordinary `Var` truthiness. Stops at the first accepted element. Nil and a null `pred` are false. | The runtime takes native Func while meta closures are evaluator callables. Use Lisp.apply through a reusable Func context for List.any; check its documented callback order, null callback and empty-input cases. |
| `List.append` | lib/list.x / List value operations | implementable with current values | Returns the concatenation of `a` and `b`. Neither input is modified. `b` becomes the shared tail of the result, so only `a`'s cells are rebuilt, O(len(a)) of them. When either side is `nil` the other side is returned as it stands. Raises: `<alloc-fail>` or `<size-limit>` while constructing the copied prefix. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.append to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.array` | lib/list.x / List value operations | implementable with current values | Returns a new `Array` holding the elements of `lst` in order. The `Array` is a fresh mutable container the caller owns and should free; the elements are shared, since they are only `Var`s. Convert when you need indexed access or in-place mutation. Raises: `<alloc-fail>` or `<size-limit>` while constructing the result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.array to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.assoc` | lib/list.x / absence and null values | needs representation decision | Returns the second value of the first association whose key equals `key`. Nil entries are skipped. A missing association and a missing second value both return `void`, so the two cases look the same here. | Missing values or null tests can distinguish runtime void, Null and empty List, which current transport can conflate. Validate the existing binding against this contract. Next: Apply the value-contract decision to List.assoc; retain separate success, absence and empty-value probes. |
| `List.caaaar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(car(car(car(value))))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.caaadr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(car(car(cdr(value))))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.caaar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(car(car(value)))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.caadar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(car(cdr(car(value))))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.caaddr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(car(cdr(cdr(value))))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.caadr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(car(cdr(value)))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.caar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(car(lst))`. Compound accessors read from right to left: `a` applies `car` and `d` applies `cdr`. Every step is `nil`-safe. | A car step may return runtime void; current adapters collapse that absence into an empty List. Validate the existing binding against this contract. Next: Use the value-contract decision before extending absent-selector adapters. |
| `List.cadaar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(cdr(car(car(value))))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.cadadr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(cdr(car(cdr(value))))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.cadar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(cdr(car(value)))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.caddar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(cdr(cdr(car(value))))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.cadddr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(cdr(cdr(cdr(value))))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.caddr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns the third element, or `void`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Validate the existing binding against this contract. Next: Use the value-contract decision before extending absent-selector adapters. |
| `List.cadr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(cdr(lst))`, or `void` when there is no second element. | A car step may return runtime void; current adapters collapse that absence into an empty List. Validate the existing binding against this contract. Next: Use the value-contract decision before extending absent-selector adapters. |
| `List.car` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Method form of `car`: the head of `lst`, or `void` when `lst` is `nil`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Validate the existing binding against this contract. Next: Use the value-contract decision before extending absent-selector adapters. |
| `List.cdaaar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `cdr(car(car(car(value))))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.cdaadr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `cdr(car(car(cdr(value))))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.cdaar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `cdr(car(car(value)))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.cdadar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `cdr(car(cdr(car(value))))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.cdaddr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `cdr(car(cdr(cdr(value))))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.cdadr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `cdr(car(cdr(value)))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.cdar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `cdr(car(value))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `List.cddaar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `cdr(cdr(car(car(value))))`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
...
| `List.equal` | lib/list.x / List value operations | implementable with current values | Reports equality of canonical chains by exact head and tail identity. It matches `List.hash`, so the same elements decide it: content for a small number, `Symbol`, `Atom`, canonical `String`, or nested `List`, and identity for a wide number boxed in a `Scope`, an `Array`, or a `Map`. Two `List`s built from one `long` value hold two boxes and are unequal, while `List.compare` reads the values and calls them equal. Use `List.compare` where content has to decide. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.equal to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.filter` | lib/list.x / List interpreted callbacks | needs callback adapter | Returns the elements `pred` accepts by ordinary `Var` truthiness. Elements are passed as values, and filtering `nil` gives `nil` without invoking or checking `pred`. Raises: whatever `Func.apply`, `pred`, or result truthiness raises, or `<alloc-fail>` or `<size-limit>` while constructing the result. A null `pred` on nonempty input raises `<bad-arg>` from `Func.apply`. | The runtime takes native Func while meta closures are evaluator callables. Validate the existing binding against this contract. Next: Use Lisp.apply through a reusable Func context for List.filter; check its documented callback order, null callback and empty-input cases. |
| `List.find` | lib/list.x / List interpreted callbacks | needs callback adapter | Returns the first element `pred` accepts by ordinary `Var` truthiness, or `void`. | The runtime takes native Func while meta closures are evaluator callables. Use Lisp.apply through a reusable Func context for List.find; check its documented callback order, null callback and empty-input cases. |
| `List.flatten` | lib/list.x / List value operations | implementable with current values | Flattens one level of nested `List`s into a canonical result. A nested `nil` contributes no element, non-`List` values retain their identity, and `nil` returns `nil`. Raises: `<alloc-fail>` or `<size-limit>` while constructing the result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.flatten to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.flatten_all` | lib/list.x / List value operations | implementable with current values | Recursively flattens every nested `List` into a canonical result. Nested `nil` contributes no element and `nil` returns `nil`. Raises: `<alloc-fail>` or `<size-limit>` while constructing the result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.flatten_all to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.foldl` | lib/list.x / seeded folds | needs representation decision | Folds `fn` over the elements of `lst` from the left, starting at `seed`. `fn` receives the accumulator and then the next element, and returns the next accumulator. A `void` seed means "no seed": the first element becomes the initial accumulator and the fold starts at the second, and folding `nil` that way returns `void`. A null `fn` returns the accumulator untouched, which for a `void` seed is the head. | Runtime void means no seed; an empty List is a distinct valid seed; null callbacks are permitted. Settle true-void transport, then bridge the callback preserving seed, order and empty-input behavior. |
| `List.get` | lib/list.x / absence and null values | needs representation decision | Looks up an integer index or association key in `list`. Integer keys use `List.getindex`, including negative indexes; every other key uses `List.assoc`. Either absent form returns `void`, as does an integer key outside the `int` index domain, which no `List` can reach. | Missing values or null tests can distinguish runtime void, Null and empty List, which current transport can conflate. Validate the existing binding against this contract. Next: Apply the value-contract decision to List.get; retain separate success, absence and empty-value probes. |
| `List.getindex` | lib/list.x / absence and null values | needs representation decision | Returns `list[index]`, or `void` when out of range. A negative index counts from the end and is found without a length pass. | Missing values or null tests can distinguish runtime void, Null and empty List, which current transport can conflate. Validate the existing binding against this contract. Next: Apply the value-contract decision to List.getindex; retain separate success, absence and empty-value probes. |
| `List.getslice` | lib/list.x / List value operations | implementable with current values | Returns `list[start:stop:step]`. `stop` is exclusive, negative bounds count from the end, and a negative `step` walks backwards. A full forward slice preserves `list` only when its identity is canonical in the active pool chain; otherwise it rebuilds the cells there so a detached pool cannot escape through the shortcut. Raises: `<bad-arg>` when `step` is zero, or `<alloc-fail>` or `<size-limit>` while constructing the result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.getslice to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.hash` | lib/list.x / List value operations | implementable with current values | Returns the stable hash of `List`'s exact head bits and tail identity. This is a constant-time hash of one cell, which canonical cells make sound. It reaches an element's content only when the `Var` word holds it: a small number, a `Symbol`, an `Atom`, a canonical `String`, or a nested `List`. An element the word points at hashes by identity, so a wide number boxed in a `Scope`, an `Array`, or a `Map` gives two equal-looking `List`s two hashes. Mutating an object referenced by the head does not change this hash. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.hash to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.head` | lib/list.x / List value operations | implementable with current values | Returns the first `count` elements of `list`. When `count` reaches or exceeds the length, `list` itself comes back, the same pointer. `List`s are immutable, so sharing it is safe. Otherwise fresh canonical cells are built for the prefix. Raises: `<alloc-fail>` or `<size-limit>` while constructing that prefix. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.head to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.index` | lib/list.x / List value operations | implementable with current values | Returns the first index of `key`, or -1 when absent. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.index to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.iter` | lib/list.x / native iterator state | needs representation decision | Initializes caller-owned `dest` as a forward iterator over `lst`. The iterator borrows the immutable cells and yields their stored `Var` bits without retaining them, so the owning pool must outlive iteration. A null `dest` returns NULL; `nil` produces an exhausted iterator. | Iter contains caller-owned state and a native next callback; collection producers are lazy. Give List.iter real bounded Iter storage and lifetime; preserve source mutation and exhaustion semantics. |
...
| `List.match_replace` | lib/match.x / List value operations | implementable with current values | Returns the `List` replacement when `input` matches `pat`. A miss returns `input` unchanged. A successful scalar replacement cannot inhabit the `List` result and returns `nil`. Matching and replacement failures follow `List.try_match_replace`. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.match_replace to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.nth_cdr` | lib/list.x / List value operations | implementable with current values | Returns the shared tail beginning `n` cells in. Returns `nil` past the end and `list` itself when `n` is nonpositive. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.nth_cdr to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
...
| `List.replace` | lib/match.x / List value operations | implementable with current values | Replaces named binders in `template` according to `bindings`. A sequence binder in list-head position splices its captured `List`; `!quote` removes itself and leaves its operand literal. Missing binders are retained. A null template returns `nil`, and null bindings return `template` unchanged. New structure follows the module pool-chain lifetime above. Raises: `<alloc-fail>` while constructing replacement `List`s. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.replace to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.repr` | lib/list.x / List value operations | implementable with current values | Returns the re-readable rendering of `lst`. This is the `%(...)` spelling of the same data. Elements are rendered with their own `repr`, so `String`s come back quoted and `Atom`s print as bare names, and `nil` renders as `()`. Long nested structure wraps at about 80 columns. The canonical result follows the active `String` pool chain, may already be owned by an ancestor, and remains live until that owner is released. Raises: `<alloc-fail>` or `<size-limit>` while constructing the result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.repr to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.reverse` | lib/list.x / List value operations | implementable with current values | Returns a new `List` holding the elements of `lst` in reverse order. Fresh cells are built through `cons`, so the result is canonical and `lst` is untouched. Reversing `nil` gives `nil`. Raises: `<alloc-fail>` while constructing the result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.reverse to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.search` | lib/match.x / List value operations | implementable with current values | Returns every matching subtree of `input` with its bindings. Each result begins with `(* matched)` followed by reverse-slot-order binder pairs. Traversal visits a `List`'s head, then tail, then the `List` itself; results are prepended and therefore returned in reverse visitation order. Explicit `nil` values are nodes, but a proper `List`'s terminal cdr is not. A miss, malformed pattern, or machine error returns `nil`. Raises: `<size-limit>` for an ineligible pattern, or `<alloc-fail>` while preparing or constructing results. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.search to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.search_replace` | lib/match.x / List value operations | implementable with current values | Replaces every matching subtree in `input` from the leaves upward. Children are rewritten before their reconstructed containing `List` is tested. A miss, malformed pattern, or machine error returns `input` unchanged. New structure follows the module pool-chain lifetime above. Raises: `<size-limit>` for an ineligible pattern, or `<alloc-fail>` while preparing, traversing, or replacing. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.search_replace to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.sort` | lib/list.x / List value operations | implementable with current values | Returns a copy of `lst` ordered by `Var.compare`. `lst` is unchanged. There is no comparator parameter. `Var.compare` orders the element tags involved, so a mixed-kind `List` still sorts. A `List` of fewer than two cells is returned as it stands. Raises: whatever element comparison raises, or `<alloc-fail>` while constructing the result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.sort to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.sort_by` | lib/list.x / List interpreted callbacks | needs callback adapter | Returns a stable sorted copy using `Array.sort_by`'s callback contract. The key runs once per element in input order; an empty List invokes none. The input List is unchanged. Raises: allocation and the key's ordinary causes, including failure to compare the resulting keys. | The runtime takes native Func while meta closures are evaluator callables. Use Lisp.apply through a reusable Func context for List.sort_by; check its documented callback order, null callback and empty-input cases. |
| `List.sort_with` | lib/list.x / List interpreted callbacks | needs callback adapter | Returns a stable sorted copy using `Array.sort_with`'s callback contract. The input List is unchanged. Fewer than two cells return unchanged without a callback. Raises: allocation and the comparator's ordinary causes. | The runtime takes native Func while meta closures are evaluator callables. Use Lisp.apply through a reusable Func context for List.sort_with; check its documented callback order, null callback and empty-input cases. |
| `List.str` | lib/list.x / List value operations | implementable with current values | Returns the human-readable rendering of `lst`. Elements are rendered with their own `str`, so a `String` element appears unquoted. The writer pads inside parentheses and breaks nested structure across lines at about 80 columns. Nil renders as `()`. The canonical result follows the active `String` pool chain, may already be owned by an ancestor, and remains live until its owning pool is released. Use `List.repr` when the text has to read back in. Raises: `<alloc-fail>` or `<size-limit>` while constructing the result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.str to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.sublis` | lib/list.x / List value operations | implementable with current values | Recursively substitutes non-`List` nodes in `tree` from `alist`. Each association is a `List` whose first value is the key and whose second value is the replacement. Unmatched leaves are shared; `List` structure is rebuilt canonically, and `nil` returns `nil`. Raises: a cause from key equality, or `<alloc-fail>` or `<size-limit>` while constructing the result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.sublis to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.subseq` | lib/list.x / List value operations | implementable with current values | Returns every `step`th element from `start` up to exclusive `stop`. Negative bounds count from the end. `step` must be positive. Raises: `<bad-arg>` when `step` is less than 1, or `<alloc-fail>` while constructing the result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.subseq to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.tail` | lib/list.x / List value operations | implementable with current values | Returns the last `count` elements of `list`. The result is an existing tail of `list`, so nothing is allocated. When `count` reaches or exceeds the length, the whole `list` comes back. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind List.tail to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `List.truth` | lib/common.x / explicit syntax operations | implementable with current values | Returns nonzero when `list` is not `nil`, the null empty List. | Syntax lowering exists separately; it does not install this explicitly named method. Validate the existing binding against this contract. Next: Bind List.truth's native operation or prove exact equivalence with its lowering, including mutation, coercion and failure results. |
...
| `Array.copy` | lib/array.x / Array value operations | implementable with current values | Returns a new `Array` holding the same elements as `array`. The copy is shallow and independent: pushing to one does not affect the other, but the two share whatever objects their elements point at. Because `Array`s are identity-bearing, plain assignment aliases instead of copying, so copy before handing scratch storage to code that may mutate it. Raises: `<size-limit>` or `<alloc-fail>` when the copy cannot be represented or allocated. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Array.copy to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Array.count` | lib/array.x / Array value operations | implementable with current values | Returns how many elements compare equal to `value`. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Array.count to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Array.equal` | lib/array.x / Array value operations | implementable with current values | Returns nonzero when two `Array`s have structurally equal elements. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Array.equal to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Array.find` | lib/array.x / absence and null values | needs representation decision | Returns the index of the first element equal to `value`, or -1. The scan is linear and compares with `Var` equality, the same rule `==` applies to boxed values. `Array`s and `Map`s participate structurally, while `===` remains the identity test. `Array.indexof` is a synonym. Raises: `<size-limit>` when `array` exceeds the `INT_MAX` index limit that `Array.getindex` describes. | Missing values or null tests can distinguish runtime void, Null and empty List, which current transport can conflate. Validate the existing binding against this contract. Next: Apply the value-contract decision to Array.find; retain separate success, absence and empty-value probes. |
| `Array.foldl` | lib/array.x / seeded folds | needs representation decision | Folds `fn` over `array` from `seed`, front to back. A `void` seed means "no seed": the first element becomes the accumulator and the fold starts at the second, so folding an empty `Array` that way returns `void`. Elements are passed as values. Input with nothing left to fold does not invoke or check `fn`. The callback receives the accumulator then the next element and is not retained. It must not structurally mutate `array` during the walk. Any cause from `Func.apply` or `fn` propagates without changing `array`. | Runtime void means no seed; an empty List is a distinct valid seed; null callbacks are permitted. Settle true-void transport, then bridge the callback preserving seed, order and empty-input behavior. |
...
| `Array.join` | lib/array.x / Array value operations | implementable with current values | Joins the elements of `array` into one `String` separated by `separator`. Elements are converted with their `str` form, so a `String` element contributes its bytes without quotes and a number contributes its digits. A null `separator` joins with nothing between elements, and an empty array yields the empty `String`. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Array.join to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Array.len` | lib/array.x / Array value operations | implementable with current values | Interface only; inspect the producer. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Array.len to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Array.list` | lib/list.x / Array value operations | implementable with current values | Returns a new `List` holding the elements of `arr` in order. Cells are built from the end backwards through `cons`, so the result is canonical and shares whatever tail it already has in common with another `List`. `arr` is neither consumed nor freed. Raises: `<alloc-fail>` while constructing the result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Array.list to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
...
| `Array.shift` | lib/array.x / absence and null values | needs representation decision | Removes and returns the first element of `array`, or `void` if it is empty. Every remaining element moves down one position, so this is O(n) in the length while `Array.take_last` is O(1). Pair `Array.push` with `shift` for a FIFO queue and with `Array.take_last` for a stack. | Missing values or null tests can distinguish runtime void, Null and empty List, which current transport can conflate. Validate the existing binding against this contract. Next: Apply the value-contract decision to Array.shift; retain separate success, absence and empty-value probes. |
| `Array.sort` | lib/array.x / Array value operations | implementable with current values | Sorts `array` in place in ascending order and returns that same `Array`. Call `Array.copy` first to preserve the original order. Ordering is `Var.compare`, which compares numbers numerically and `String`s, `Symbol`s, and `Atom`s by content. The sort is `qsort`, so it is not stable, and a null or one-element `array` is returned unchanged. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Array.sort to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Array.sort_by` | lib/array.x / Array interpreted callbacks | needs callback adapter | Stably sorts `array` by keys produced once per value, returning the Array. The borrowed callback runs front to back and must not mutate this Array. Keys use `Var.compare`; equal keys preserve the original element order. A null or empty Array invokes no callback. Sorting another Array inside the callback is valid. Raises: allocation, key comparison, `Func.apply`, or callback causes. Failure leaves the original Array unchanged; callback side effects are not undone. | The runtime takes native Func while meta closures are evaluator callables. Use Lisp.apply through a reusable Func context for Array.sort_by; check its documented callback order, null callback and empty-input cases. |
| `Array.sort_with` | lib/array.x / Array interpreted callbacks | needs callback adapter | Stably sorts `array` with a borrowed synchronous comparator and returns it. The callback receives two values; its result converts to `int`, with a negative, zero, or positive result ordering the first before, equal to, or after the second. It must give a consistent ordering and must not mutate this Array. Sorting a different Array inside the callback is valid. Null and fewer than two elements return unchanged without a callback. Raises: allocation, `Func.apply`, conversion, or callback causes. Failure leaves the original Array unchanged; temporary storage is released. Callback side effects are not undone. | The runtime takes native Func while meta closures are evaluator callables. Use Lisp.apply through a reusable Func context for Array.sort_with; check its documented callback order, null callback and empty-input cases. |
...
| `Map.get` | lib/map.x / absence and null values | needs representation decision | Returns the value stored under `key`, or `void` when absent. A convenience over `Map.try_get`, kept because it reads well inside a larger expression. `void` as the missing-key answer is unambiguous because `Map.set` refuses to store `void`. It says nothing else about the miss, so prefer `Map.try_get` where absence has to be handled. | Missing values or null tests can distinguish runtime void, Null and empty List, which current transport can conflate. Validate the existing binding against this contract. Next: Apply the value-contract decision to Map.get; retain separate success, absence and empty-value probes. |
| `Map.get_hashed` | lib/map.x / absence and null values | needs representation decision | Returns the value stored under `key`, or `void` when absent, probing with the caller's precomputed `key_hash`. `key_hash` must be `Var.hash` of `key`; another value reports the key as absent. This serves a caller that probes several `Map`s with one key, such as `Pool.lookup` walking its chain, and is `Map.get` in every other respect. A null `Map` reports absence. Raises: a cause raised by custom key equality. Hashing happens in the caller, so a `void` key raises there instead. | Missing values or null tests can distinguish runtime void, Null and empty List, which current transport can conflate. Apply the value-contract decision to Map.get_hashed; retain separate success, absence and empty-value probes. |
| `Map.getdefault` | lib/map.x / Map value operations | implementable with current values | Returns the value stored under `key`, or `defval` when it is absent. Nothing is inserted and `Map.len` does not change, unlike `Map.setdefault`. For counting, numeric `map[k] += amount` initializes an absent key from `amount`; use `getdefault` when a read needs a fallback without mutation. Raises: `<void-op>` when `key` is `void`, or a cause raised by custom key hashing or equality. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Map.getdefault to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Map.getindex` | lib/map.x / absence and null values | needs representation decision | Returns the value selected by bracket indexing. This is what `map[key]` lowers to, and it is `Map.get` in every respect, including identity comparison for `Array` and `Map` keys. Bracket reads do not report status; prefer `Map.try_get` when an absent key and a `void` result must be told apart. | Missing values or null tests can distinguish runtime void, Null and empty List, which current transport can conflate. Validate the existing binding against this contract. Next: Apply the value-contract decision to Map.getindex; retain separate success, absence and empty-value probes. |
| `Map.iter` | lib/map.x / native iterator state | needs representation decision | Initializes `dest` as an iterator over `x`, yielding each value. Keys come from `Map.enumerate`, which pairs each one with its value the way `Iter.enumerate` pairs an index with an element. | Iter contains caller-owned state and a native next callback; collection producers are lazy. Give Map.iter real bounded Iter storage and lifetime; preserve source mutation and exhaustion semantics. |
| `Map.keys` | lib/map.x / native iterator state | needs representation decision | Initializes `dest` as an iterator over `x`, yielding each key. The mirror of `Map.iter`, and like it, it allocates nothing. | Iter contains caller-owned state and a native next callback; collection producers are lazy. Give Map.keys real bounded Iter storage and lifetime; preserve source mutation and exhaustion semantics. |
| `Map.len` | lib/map.x / Map value operations | implementable with current values | Returns the number of key/value pairs in `map`. O(1), and the emptiness test for a `Map`, because an empty `Map` is a nonnull object. This counts live entries. The table keeps more slots than entries to hold its load factor, so `Map.len` is not the allocation size. `map` must be nonnull. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Map.len to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Map.list` | lib/list.x / Map value operations | implementable with current values | Returns the entries of `map` as a `List` of two-element `(key value)` `List`s, in the map's iteration order. This is the eager form of `Map.enumerate`, for a caller that wants the pairs as ordinary `List` data rather than a cursor. An empty `map` returns nil. The pair cells are new; the keys and values are shared. Raises: `<alloc-fail>` or `<size-limit>` while constructing the result. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Map.list to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
...
| `Symbol.var` | lib/common.x / Symbol value operations | implementable with current values | Boxes a `Symbol` value as `Var`. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Symbol.var to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
...
| `Var.array` | lib/common.x / boxed value extraction | implementable with current values | Extracts the `Array` payload of `value`, or NULL for another tag. | The native extractor decodes a represented payload. Pointer forms borrow storage and do not prove the target family or lifetime. Validate the existing binding against this contract. Next: Preserve Var.array's caller preconditions; test valid typed handles and shared mutation, and inspect raw mismatch results without dereferencing them. |
...
| `Var.caaadr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(car(car(cdr(value))))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `Var.caaar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(car(car(value)))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `Var.caadar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(car(cdr(car(value))))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `Var.caaddr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(car(cdr(cdr(value))))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `Var.caadr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(car(cdr(value)))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `Var.caar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Applies the `caar` selector chain to `Var`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Validate the existing binding against this contract. Next: Use the value-contract decision before extending absent-selector adapters. |
| `Var.cadaar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(cdr(car(car(value))))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `Var.cadadr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(cdr(car(cdr(value))))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `Var.cadar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(cdr(car(value)))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `Var.caddar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(cdr(cdr(car(value))))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `Var.cadddr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `car(cdr(cdr(cdr(value))))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `Var.caddr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Applies the `caddr` selector chain to `Var`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Validate the existing binding against this contract. Next: Use the value-contract decision before extending absent-selector adapters. |
| `Var.cadr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Applies the `cadr` selector chain to `Var`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Validate the existing binding against this contract. Next: Use the value-contract decision before extending absent-selector adapters. |
| `Var.car` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Treats `var` as a `List` and returns its first element. | A car step may return runtime void; current adapters collapse that absence into an empty List. Validate the existing binding against this contract. Next: Use the value-contract decision before extending absent-selector adapters. |
| `Var.cdaaar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `cdr(car(car(car(value))))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `Var.cdaadr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `cdr(car(car(cdr(value))))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `Var.cdaar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `cdr(car(car(value)))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `Var.cdadar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `cdr(car(cdr(car(value))))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `Var.cdaddr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `cdr(car(cdr(cdr(value))))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `Var.cdadr` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `cdr(car(cdr(value)))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `Var.cdar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `cdr(car(value))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
| `Var.cddaar` | lib/list-selectors.x; lib/list.x / canonical List selectors | needs representation decision | Returns `cdr(cdr(car(car(value))))` after treating `value` as a `List`. | A car step may return runtime void; current adapters collapse that absence into an empty List. Use the value-contract decision before extending absent-selector adapters. |
...
| `Var.float` | lib/common.x / Var value operations | implementable with current values | Returns `x` as a native `float` under the `Var.convert` rules. Raises: `Var.convert`'s causes when the source is nonnumeric, `void`, or invalidly encoded. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.float to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Var.floating` | lib/var.x / Var value operations | implementable with current values | Returns `v`'s payload as a `double` when its tag is floating, or 0.0. Handles `<f32>`, `<f64>`, and `<ldouble>`, and reconstructs NaN, `+Inf`, and `-Inf` from their discrete tags. An `<ldouble>` payload is narrowed to `double`; call `Var.long_double_value` to keep the full precision. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.floating to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Var.getindex` | lib/dispatch.x / absence and null values | needs representation decision | Reads a dynamic indexed value through the receiver's protocol row. Raises: `<bad-enc>`, `<void-op>`, or `<no-member>` when the dynamic receiver cannot be indexed. | Missing values or null tests can distinguish runtime void, Null and empty List, which current transport can conflate. Apply the value-contract decision to Var.getindex; retain separate success, absence and empty-value probes. |
...
| `Var.is_binder` | lib/match.x / Var value operations | implementable with current values | Reports whether `atom` is either valid `Match` binder form. Raises: `<alloc-fail>` while decoding a compact `Atom`. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.is_binder to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Var.is_floating` | lib/var.x / Var value operations | implementable with current values | Reports whether `v` belongs to the floating runtime family. True for `<f32>`, `<f64>`, and `<ldouble>`, and for the discrete `<nan>`, `<+inf>`, and `<-inf>` values. Read the payload with `Var.floating`, or with `Var.long_double_value` when the tag is `<ldouble>` and the extra precision matters. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.is_floating to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Var.is_integer` | lib/var.x / Var value operations | implementable with current values | Reports whether `v` belongs to the integer runtime family. True for every signed and unsigned integer family from `<u8>` through `<ullong>`, including the scope-owned wide boxes. `Symbol`s are a kind of their own and answer 0 here, even though `Var.integer` returns a `Symbol`'s numeric value. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.is_integer to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Var.is_list_binder` | lib/match.x / Var value operations | implementable with current values | Reports whether `atom` is a valid named or anonymous `*` binder. Raises: `<alloc-fail>` while decoding a compact `Atom`. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.is_list_binder to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Var.is_match_op` | lib/match.x / Var value operations | implementable with current values | Reports whether `atom` is a compact built-in `Match` guard operator. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.is_match_op to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
...
| `Var.long_long` | lib/common.x / Var value operations | implementable with current values | Returns `x` as a native `long long` under the `Var.convert` rules. Raises: `Var.convert`'s causes when the source is nonnumeric, `void`, out of floating range, or invalidly encoded. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.long_long to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Var.long_long_value` | lib/var.x / Var value operations | implementable with current values | Returns an `<llong>` box's signed payload, or 0 for another tag. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.long_long_value to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Var.long_value` | lib/var.x / Var value operations | implementable with current values | Returns the payload of a `<long>` box, or 0 if `v` has a different tag. The test is on the tag, so a `<ulong>`, `<llong>`, or `<i32>` value reads as 0 with no complaint. `Var.integer` accepts any integer family; use this one when the tag is known and the payload must survive intact. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.long_value to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Var.map` | lib/common.x / boxed value extraction | implementable with current values | Extracts the `Map` payload of `value`, or NULL for another tag. | The native extractor decodes a represented payload. Pointer forms borrow storage and do not prove the target family or lifetime. Validate the existing binding against this contract. Next: Preserve Var.map's caller preconditions; test valid typed handles and shared mutation, and inspect raw mismatch results without dereferencing them. |
...
| `Var.repr` | lib/dispatch.x / Var value operations | implementable with current values | Returns the readable representation of `Var`. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.repr to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
...
| `Var.uchar` | lib/common.x / Var value operations | implementable with current values | Returns `x` as a native `uchar` under the `Var.convert` rules. Raises: `Var.convert`'s causes when the source is nonnumeric, `void`, out of floating range, or invalidly encoded. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.uchar to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Var.uint` | lib/common.x / Var value operations | implementable with current values | Returns `x` as a native `uint` under the `Var.convert` rules. Raises: `Var.convert`'s causes when the source is nonnumeric, `void`, out of floating range, or invalidly encoded. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.uint to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Var.ulong` | lib/common.x / Var value operations | implementable with current values | Returns `x` as a native `ulong` under the `Var.convert` rules. Raises: `Var.convert`'s causes when the source is nonnumeric, `void`, out of floating range, or invalidly encoded. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.ulong to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Var.ulong_long` | lib/common.x / Var value operations | implementable with current values | Returns `x` as a native `unsigned long long` under the `Var.convert` rules. Raises: `Var.convert`'s causes when the source is nonnumeric, `void`, out of floating range, or invalidly encoded. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.ulong_long to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Var.ulong_long_value` | lib/var.x / Var value operations | implementable with current values | Returns a `<ullong>` box's unsigned payload, or 0 for another tag. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.ulong_long_value to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Var.ulong_value` | lib/var.x / Var value operations | implementable with current values | Returns the payload of a `<ulong>` box, or 0 if `v` has a different tag. Prefer this to `Var.integer` for a `<ulong>`: the general reader casts the payload to `long`, which reinterprets anything above `LONG_MAX` as negative, while this reader returns the unsigned value. A tag that is not `<ulong>` yields 0 silently, so test with `Var.is` when the tag is in doubt. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.ulong_value to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
| `Var.unsigned` | lib/common.x / Var value operations | implementable with current values | Returns `x` as a native `unsigned` under the `Var.convert` rules. Raises: `Var.convert`'s causes when the source is nonnumeric, `void`, out of floating range, or invalidly encoded. | The signature uses represented values and an existing runtime owner. Validate the existing binding against this contract. Next: Bind Var.unsigned to its native owner and compare normal, empty and boundary inputs without changing the public contract. |
