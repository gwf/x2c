...
`etc/compiler-sdk.xlisp`. `X2C_HOME` sets the home explicitly and takes
precedence.
...
home rather than under `/mnt/c`, so builds run at native speed. Use clang
15 or newer, or `gcc`: Ubuntu 22.04 ships clang 14, which rejects two static
initializers in the runtime.
...
workflow's `windows_spike` input tests both routes. Native Windows without a
POSIX layer is not supported, and under MSYS2 only a source build is
supported.
...
include root and write the runtime header as `<x2c/x2c.h>`, so every system
...
C generated by `x2c translate` keeps its own quoted includes. To compile it
by hand, pass `-iquote "$HOME/.local/x2c/include/x2c"` instead. `x2c build`
passes that option itself.
