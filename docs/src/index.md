...
- `macro` generates hygienic, parsed, typed source at compile time. `Type`
  ...
  [Automatic differentiation](guide/autodiff.md) is built entirely this
  way: dual-number families from a `Type` hole, and forward- and
  reverse-mode derivative functions generated by decorators.
