...
`lib/meta.x`, so a compile-time function that includes it calls the compiler
...
definition anywhere. A compile-time function that reaches one, directly or
through another compile-time function, therefore has no runtime form and none
is emitted for it; the compiler derives this and needs no spelling for it.
Calling such a function at run time is a link error that names it.
