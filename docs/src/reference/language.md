...
- The function is also emitted as C and behaves as an ordinary function at run
  time, unless it reaches a `Meta` operation, directly or through another
...
The name is mechanical: drop the `x2c.` prefix, put the rest under `Meta`, and
replace each remaining `.` with `_`. `x2c.type.fields` is `Meta.type_fields`,
`x2c.invocation.line` is `Meta.invocation_line`, `x2c.ident` is `Meta.ident`.
Two answers differ in shape because x2c has no spelling for the Lisp one:

- `Meta.expr_call(List callee, List arguments)` takes its arguments as one
  `List` rather than as a rest parameter.
- `Meta.type_value(List value)` returns `int`, 1 or 0, rather than a Lisp
  truth value.

`x2c.comptime.install` and `x2c.comptime.lower` have no `Meta` spelling,
because they run the pass that translates the calling function.
