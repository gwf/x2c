...
two of them define one name, the first supplies it and the compiler warns. Modules work on macOS,
...
corresponding defaults. `--target <name>` builds the named manifest target and `--profile
...
- `warning` covers the remaining cases: an unnecessary conversion, a
  protocol binder that shadows a visible type name, and a native module
  function that a function the compiler links shadows.
