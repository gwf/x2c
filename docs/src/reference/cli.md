...
loading compiler's copy, which is the whole runtime, and it records that
...
Translation also reports warnings, which never stop a build: the unit
compiles and the program runs. Warnings do not count against `--max-errors`,
and there is no option to suppress or escalate one. A warning describes
...
- `region` says a value allocated inside a region can still be reached after
  ...
  opened the region.
- `after-free` says a local was read after `Scope.free`, `Scope.realloc`,
  or `Array.list_free` consumed it.
- `bad-free` says `Scope.free` or `Scope.realloc` was given a literal, a
  local's address, or other storage no Scope allocator returned.
...
input, and Make recipes show it too. Redirected standard error also uses stable
