...
| Function | Summary |
| --- | --- |
| [`Buffer.cleanup`](#Buffer.cleanup) | Ends the owned lifetime when a managed local leaves its block. |
...
<a id="Buffer.cleanup"></a>
#### Buffer.cleanup

`void Buffer.cleanup(Buffer value)`
...
`meta native Self Buffer.newline(Self buf)`
...
`meta native Self Buffer.newline_indent(Self buf)`
...
`meta native Self Buffer.pad(Self buf)`
