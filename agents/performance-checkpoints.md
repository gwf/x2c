...
builds one stage from HEAD with `make bm-build-scaling`, counts the
instructions its translator and C compiler processes retire, and divides by
the source lines in `src/` and `lib/`. The score is that cost per line as a
percentage of the baseline in
`unittest/benchmarks/build-scaling-baseline.json`.

Read the first lines of `latest.md`. A score that rose 2 or more points since
the previous snapshot means the commits in between made each line more
expensive to build; find the commit that caused it. Repeat runs of one tree
agree within 0.2 points, even under heavy host load. The score assumes build
cost grows in proportion to source lines.

The instruction count does not see cache or memory stalls, and a new C
compiler changes it. After a toolchain upgrade, run
