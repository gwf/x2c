 ...
 *     --output /tmp/mandelbrot-ported ported.x
 *   /tmp/mandelbrot-ported 500 500 1
 */
...
int main(int argc, char **argv) {
  ...
  for (int run = atoi(argv[3]); run > 0; run--) {
    ...
    /* A PBM row is eight pixels to the byte, leftmost pixel in the high
     * bit, and a row that does not fill its last byte pads it on the
     * right. Walking the row a byte at a time says that outright. */
