...
int x2c_normalize_index(int index, int length){
  if(! _init_guard_) _file_init_();
  if(length < 0){
    static const X2CErrorSite _x2c_error_site_0 = {.file = "../../lib/common.x",.function = "x2c_normalize_index",.line = 845};
...
int x2c_normalize_slice(int * start, int * stop, int step, int length){
  ...
  if(! start || ! stop || ! step || length < 0){
    static const X2CErrorSite _x2c_error_site_1 = {.file = "../../lib/common.x",.function = "x2c_normalize_slice",.line = 865};
