...
_x2c_initializer_choice_6E6B8BB0_4((x2c_repo_cpp_include_dirs = NULL))
...
__attribute__((constructor)) static void _file_init_(void){
  ...
  _24 = String_new("etc/compiler-sdk.xlisp");
...
int x2c_source_file(String path){
  if(! _init_guard_) _file_init_();
  if(String_endswith(path, _16)) return 1;
  if(String_endswith(path, _17) || String_endswith(path, _18) || String_endswith(path, _19) || String_endswith(path, _20)) return 0;
...
String x2c_find_program(String name){
  ...
  {
    String directory;
    List _x2c_macro_object_1 = String_split(Env_get(_22), _21);
...
static int _is_home(Path p){
  return Path_is_dir(Path_join(p, _23)) && Path_is_file(Path_join(p, _24));
}
