...
_x2c_initializer_choice_29BB659F_0((0 < sizeof(cli_options) / sizeof(cli_options[0])), (cli_options[0] =(CliOption){
  535328, CLI_TOP | CLI_TRANSLATE | CLI_NATIVE | CLI_BOOTSTRAP | CLI_ENV | CLI_INSTALL | CLI_REMOVE | CLI_LIST | CLI_NEW, 15397654616, _48, NULL, "Show help and exit", 0, .alias = _47
}
), (1ULL < sizeof(cli_options) / sizeof(cli_options[1ULL])), (cli_options[1ULL] =(CliOption){
  47619197916, CLI_TOP, 495915096, _50, NULL, "Show the x2c version and exit", 0, .alias = _49
}
), (2ULL < sizeof(cli_options) / sizeof(cli_options[2ULL])), (cli_options[2ULL] =(CliOption){
  47618096330, CLI_TRANSLATE | CLI_NATIVE | CLI_BOOTSTRAP, 15397654616, _53, NULL, "Show commands as they are executed", 0, .alias = _54
}
), (3ULL < sizeof(cli_options) / sizeof(cli_options[3ULL])), (cli_options[3ULL] =(CliOption){
  9852392796, CLI_TRANSLATE | CLI_NATIVE, 15397654616, _55, NULL, "Show commands without executing them", 0
}
), (4ULL < sizeof(cli_options) / sizeof(cli_options[4ULL])), (cli_options[4ULL] =(CliOption){
  37046632, CLI_TRANSLATE | CLI_BUILD | CLI_RUN | CLI_BOOTSTRAP | CLI_INSTALL | CLI_REMOVE | CLI_NEW, 15397654616, _56, NULL, "Suppress successful progress and receipts", 0, .alias = _57
}
), (5ULL < sizeof(cli_options) / sizeof(cli_options[5ULL])), (cli_options[5ULL] =(CliOption){
  34343516, CLI_TRANSLATE | CLI_NATIVE | CLI_BOOTSTRAP, 15397654616, _58, NULL, "Use stable output without terminal rendering", 0
}
), (6ULL < sizeof(cli_options) / sizeof(cli_options[6ULL])), (cli_options[6ULL] =(CliOption){
  7300068, CLI_TRANSLATE | CLI_NATIVE | CLI_BOOTSTRAP, 15397654616, _59, "<auto|always|never>", "Control terminal color", 0
}
), (7ULL < sizeof(cli_options) / sizeof(cli_options[7ULL])), (cli_options[7ULL] =(CliOption){
  8721742, CLI_TRANSLATE | CLI_NATIVE, 15397654616, _60, NULL, "Enable compiler debug logging", 0
}
), (8ULL < sizeof(cli_options) / sizeof(cli_options[8ULL])), (cli_options[8ULL] =(CliOption){
  918708911504550, CLI_TRANSLATE | CLI_NATIVE, 15397654616, _61, "<count>", "Stop after <count> errors per unit (default: 20)", 0
}
), (9ULL < sizeof(cli_options) / sizeof(cli_options[9ULL])), (cli_options[9ULL] =(CliOption){
  9417250982666, CLI_TRANSLATE | CLI_NATIVE, 15397654616, _62, "<file>", "Write compiler diagnostics to <file> as JSON Lines", 0
}
), (10ULL < sizeof(cli_options) / sizeof(cli_options[10ULL])), (cli_options[10ULL] =(CliOption){
  33665524324, CLI_TRANSLATE, 1052018024, _63, "<dir>", "Write generated files under <dir> (default: .)", 0
}
), (11ULL < sizeof(cli_options) / sizeof(cli_options[11ULL])), (cli_options[11ULL] =(CliOption){
  42018498656, CLI_TRANSLATE | CLI_NATIVE, 1052018024, _64, NULL, "Map generated C locations to original x2c sources", 0
}
), (12ULL < sizeof(cli_options) / sizeof(cli_options[12ULL])), (cli_options[12ULL] =(CliOption){
  31136689190, CLI_TRANSLATE, 1052018024, _65, NULL, "Do not write x2c dependency files", 0
}
), (13ULL < sizeof(cli_options) / sizeof(cli_options[13ULL])), (cli_options[13ULL] =(CliOption){
  286754491146, CLI_TRANSLATE, 1052018024, _66, "<file>", "Override the depfile path (one input only)", 0
}
), (14ULL < sizeof(cli_options) / sizeof(cli_options[14ULL])), (cli_options[14ULL] =(CliOption){
  293637522078056, CLI_TRANSLATE, 1052018024, _67, "<target>", "Override the depfile target (one input only)", 0
}
), (15ULL < sizeof(cli_options) / sizeof(cli_options[15ULL])), (cli_options[15ULL] =(CliOption){
  996399414194, CLI_TRANSLATE, 1052018024, _68, NULL, "Omit phony rules for included files", 0
}
), (16ULL < sizeof(cli_options) / sizeof(cli_options[16ULL])), (cli_options[16ULL] =(CliOption){
  896459484392, CLI_BUILD | CLI_RUN, 1345468776, _69, "<file>", "Use <file> instead of discovering x2c.toml", 0
}
), (17ULL < sizeof(cli_options) / sizeof(cli_options[17ULL])), (cli_options[17ULL] =(CliOption){
  1345468776, CLI_BUILD | CLI_RUN, 1345468776, _70, "<name>", "Build the named manifest target", 0
}
), (18ULL < sizeof(cli_options) / sizeof(cli_options[18ULL])), (cli_options[18ULL] =(CliOption){
  35599567626, CLI_BUILD | CLI_RUN, 1345468776, _71, "<name>", "Apply the named manifest build profile", 0
}
), (19ULL < sizeof(cli_options) / sizeof(cli_options[19ULL])), (cli_options[19ULL] =(CliOption){
  740232, CLI_BUILD, 1345468776, _72, "<kind>", "executable or static-library", 0
}
), (20ULL < sizeof(cli_options) / sizeof(cli_options[20ULL])), (cli_options[20ULL] =(CliOption){
  7477414666, CLI_BUILD, 1345468776, _73, NULL, "Produce object files without linking", 0, .alias = _74
}
), (21ULL < sizeof(cli_options) / sizeof(cli_options[21ULL])), (cli_options[21ULL] =(CliOption){
  686246, CLI_TRANSLATE | CLI_NATIVE, 1345468776, _75, "<count>", "Maximum parallel translation and compilation jobs", 0, .alias = _76
}
), (22ULL < sizeof(cli_options) / sizeof(cli_options[22ULL])), (cli_options[22ULL] =(CliOption){
  1052018024, CLI_BUILD | CLI_RUN, 1052018024, _77, "<file>", "Name the executable, library, or single object", 0
}
), (23ULL < sizeof(cli_options) / sizeof(cli_options[23ULL])), (cli_options[23ULL] =(CliOption){
  38995839752, CLI_SCRIPT, 1052018024, _78, NULL, "Build the script even when its cached executable is current", 0
}
), (24ULL < sizeof(cli_options) / sizeof(cli_options[24ULL])), (cli_options[24ULL] =(CliOption){
  7088220, CLI_SCRIPT, 1052018024, _79, NULL, "Remove the script's cached build and exit without running it", 0
}
), (25ULL < sizeof(cli_options) / sizeof(cli_options[25ULL])), (cli_options[25ULL] =(CliOption){
  5861298610788, CLI_BUILD | CLI_RUN, 1052018024, _80, "<dir>", "Store generated C, objects, deps, and state here", 0
}
), (26ULL < sizeof(cli_options) / sizeof(cli_options[26ULL])), (cli_options[26ULL] =(CliOption){
  6551812, CLI_BUILD | CLI_RUN, 1052018024, _81, "<file>", "Write native compile commands and retain generated files", 0
}
), (27ULL < sizeof(cli_options) / sizeof(cli_options[27ULL])), (cli_options[27ULL] =(CliOption){
  41897807850336, CLI_BUILD | CLI_RUN, 1052018024, _82, NULL, "Keep generated C and other intermediate files", 0, .label = _83
}
), (28ULL < sizeof(cli_options) / sizeof(cli_options[28ULL])), (cli_options[28ULL] =(CliOption){
  7958982899053, CLI_INSTALL, 34433862090, _84, "<hex>", "Require this digest of a downloaded or local archive", 0
}
), (29ULL < sizeof(cli_options) / sizeof(cli_options[29ULL])), (cli_options[29ULL] =(CliOption){
  19800432, CLI_INSTALL | CLI_BUILD | CLI_RUN, 34433862090, _85, "<url-or-path>", "Resolve package names through this index", 0
}
), (30ULL < sizeof(cli_options) / sizeof(cli_options[30ULL])), (cli_options[30ULL] =(CliOption){
  13603018, CLI_INSTALL, 34433862090, _86, NULL, "Install a bundle built for another x2c version", 0
}
), (31ULL < sizeof(cli_options) / sizeof(cli_options[31ULL])), (cli_options[31ULL] =(CliOption){
  1111831152, CLI_BOOTSTRAP, 1052018024, _87, "<dir>", "Install native x2c and sources under <dir>", 0
}
), (32ULL < sizeof(cli_options) / sizeof(cli_options[32ULL])), (cli_options[32ULL] =(CliOption){
  20273998090, CLI_TRANSLATE | CLI_NATIVE, 1307939018, _88, "<dir>", "Add a shared x2c/C include directory", 0
}
), (33ULL < sizeof(cli_options) / sizeof(cli_options[33ULL])), (cli_options[33ULL] =(CliOption){
  54927135910154, CLI_TRANSLATE | CLI_NATIVE, 1307939018, _89, "<dir>", "Add an x2c-only include directory", 0
}
), (34ULL < sizeof(cli_options) / sizeof(cli_options[34ULL])), (cli_options[34ULL] =(CliOption){
  8747647543562, CLI_NATIVE, 1307939018, _90, "<dir>", "Add a C-only ordinary include directory", 0
}
), (35ULL < sizeof(cli_options) / sizeof(cli_options[35ULL])), (cli_options[35ULL] =(CliOption){
  274059207002, CLI_NATIVE, 1307939018, _91, "<dir>", "Add a C-only system include directory", 0
}
), (36ULL < sizeof(cli_options) / sizeof(cli_options[36ULL])), (cli_options[36ULL] =(CliOption){
  35114656356, CLI_TRANSLATE | CLI_NATIVE | CLI_ENV, 1307939018, _92, "<dir>", "Add a directory of x2c packages", 0
}
), (37ULL < sizeof(cli_options) / sizeof(cli_options[37ULL])), (cli_options[37ULL] =(CliOption){
  973020192, CLI_TRANSLATE | CLI_BUILD | CLI_RUN, 1307939018, _93, NULL, "Skip symbol collection and preprocessing", 0
}
), (38ULL < sizeof(cli_options) / sizeof(cli_options[38ULL])), (cli_options[38ULL] =(CliOption){
  27054400850790, CLI_TRANSLATE | CLI_BUILD | CLI_RUN, 1307939018, _94, NULL, "Collect symbols through the host preprocessor", 0
}
), (39ULL < sizeof(cli_options) / sizeof(cli_options[39ULL])), (cli_options[39ULL] =(CliOption){
  241658219366, CLI_TRANSLATE | CLI_BUILD | CLI_RUN, 1307939018, _95, NULL, "Use CPP collection for this translation", 0
}
), (40ULL < sizeof(cli_options) / sizeof(cli_options[40ULL])), (cli_options[40ULL] =(CliOption){
  198, CLI_NATIVE | CLI_BOOTSTRAP | CLI_ENV, 279515230724452, _96, "<program>", "Use <program> as the host C compiler", 0
}
), (41ULL < sizeof(cli_options) / sizeof(cli_options[41ULL])), (cli_options[41ULL] =(CliOption){
  100, CLI_BUILD | CLI_BOOTSTRAP | CLI_ENV, 279515230724452, _97, "<program>", "Use <program> as the static-library archiver", 0
}
), (42ULL < sizeof(cli_options) / sizeof(cli_options[42ULL])), (cli_options[42ULL] =(CliOption){
  31784, CLI_NATIVE | CLI_BOOTSTRAP, 279515230724452, _98, NULL, "Set C optimization", 0, .label = _99, .prefix = 1
}
), (43ULL < sizeof(cli_options) / sizeof(cli_options[43ULL])), (cli_options[43ULL] =(CliOption){
  14, CLI_NATIVE, 279515230724452, _100, NULL, "Emit debug information", 0
}
), (44ULL < sizeof(cli_options) / sizeof(cli_options[44ULL])), (cli_options[44ULL] =(CliOption){
  279333770, CLI_NATIVE, 279515230724452, _101, "<name>[=<value>]", "Define a C preprocessor macro", 0
}
), (45ULL < sizeof(cli_options) / sizeof(cli_options[45ULL])), (cli_options[45ULL] =(CliOption){
  1473453116298, CLI_NATIVE, 279515230724452, _102, "<name>", "Undefine a C preprocessor macro", 0
}
), (46ULL < sizeof(cli_options) / sizeof(cli_options[46ULL])), (cli_options[46ULL] =(CliOption){
  49350, CLI_NATIVE, 279515230724452, _103, "<arg>", "Pass one argument only to C compilation", 0
}
), (47ULL < sizeof(cli_options) / sizeof(cli_options[47ULL])), (cli_options[47ULL] =(CliOption){
  26380018276, CLI_NATIVE, 825121124, _104, "<dir>", "Add a library search directory", 0
}
), (48ULL < sizeof(cli_options) / sizeof(cli_options[48ULL])), (cli_options[48ULL] =(CliOption){
  26379160754, CLI_NATIVE, 825121124, _105, "<name>", "Link library <name>", 0
}
), (49ULL < sizeof(cli_options) / sizeof(cli_options[49ULL])), (cli_options[49ULL] =(CliOption){
  38800656, CLI_NATIVE, 825121124, _106, "<dir>", "Search <dir> for shared libraries when the program runs", 0
}
), (50ULL < sizeof(cli_options) / sizeof(cli_options[50ULL])), (cli_options[50ULL] =(CliOption){
  1496, CLI_NATIVE, 825121124, _107, NULL, "Pass comma-separated arguments to the linker", 0, .label = _108, .prefix = 1
}
), (51ULL < sizeof(cli_options) / sizeof(cli_options[51ULL])), (cli_options[51ULL] =(CliOption){
  35719882824, CLI_NATIVE, 279515230724452, _109, NULL, "Enable native threading for compilation and linking", 0
}
), (52ULL < sizeof(cli_options) / sizeof(cli_options[52ULL])), (cli_options[52ULL] =(CliOption){
  14434122038422, CLI_NATIVE, 825121124, _110, "<name>", "Link a native framework on macOS", 0
}
), (53ULL < sizeof(cli_options) / sizeof(cli_options[53ULL])), (cli_options[53ULL] =(CliOption){
  52364728676, CLI_NATIVE, 825121124, _111, "<arg>", "Pass one argument to the linker", 0
}
), (54ULL < sizeof(cli_options) / sizeof(cli_options[54ULL])), (cli_options[54ULL] =(CliOption){
  1374366630, CLI_TRANSLATE, 665445396138972, _112, NULL, "Print source tokens and stop", 0
}
), (55ULL < sizeof(cli_options) / sizeof(cli_options[55ULL])), (cli_options[55ULL] =(CliOption){
  320883072032, CLI_TRANSLATE, 665445396138972, _113, NULL, "Print host-preprocessed text and stop", 0
}
), (56ULL < sizeof(cli_options) / sizeof(cli_options[56ULL])), (cli_options[56ULL] =(CliOption){
  320883072032, CLI_TRANSLATE, 665445396138972, _114, NULL, "Alias for --dump-cpp", 0
}
), (57ULL < sizeof(cli_options) / sizeof(cli_options[57ULL])), (cli_options[57ULL] =(CliOption){
  247458062609318, CLI_TRANSLATE, 665445396138972, _115, NULL, "Print host-preprocessed tokens and stop", 0
}
...
__attribute__((constructor)) static void _file_init_(void){
  ...
  _24 = String_new("unknown help command \'");
  _25 = String_new("\'");
  _26 = String_new("option takes no value \'");
  _27 = String_new("option requires a value \'");
  _28 = String_new("executable");
  _29 = String_new("static-library");
  _30 = String_new("shared-library");
  _31 = String_new("unknown target kind \'");
  _32 = String_new("invalid ");
  _33 = String_new(" \'");
  _34 = String_new("auto");
  _35 = String_new("always");
  _36 = String_new("never");
  _37 = String_new("invalid color mode \'");
  _38 = String_new("C dependency option is driver-owned \'");
  _39 = String_new("-Wl,-rpath,");
  _40 = String_new("unsupported package native argument \'");
  _41 = String_new("-");
  _42 = String_new("--");
  _43 = String_new("unexpected bootstrap operand \'");
  _44 = String_new("-o");
  _45 = String_new("unknown option \'");
  _46 = String_new(" requires exactly one operand");
  _47 = String_new("--help");
  _48 = String_new("-h");
  _49 = String_new("--version");
  _50 = String_new("-V");
  _51 = String_new("help");
  _52 = String_new("unknown command or global option \'");
  _53 = String_new("-v");
  _54 = String_new("--verbose");
  _55 = String_new("-###");
  _56 = String_new("-q");
  _57 = String_new("--quiet");
  _58 = String_new("--plain");
  _59 = String_new("--color");
  _60 = String_new("--debug");
  _61 = String_new("--max-errors");
  _62 = String_new("--diagnostics-file");
  _63 = String_new("--out-dir");
  _64 = String_new("--source-map");
  _65 = String_new("--no-deps");
  _66 = String_new("--dep-file");
  _67 = String_new("--dep-target");
  _68 = String_new("--no-phony-deps");
  _69 = String_new("--manifest-path");
  _70 = String_new("--target");
  _71 = String_new("--profile");
  _72 = String_new("--kind");
  _73 = String_new("-c");
  _74 = String_new("--compile-only");
  _75 = String_new("-j");
  _76 = String_new("--jobs");
  _77 = String_new("--output");
  _78 = String_new("--rebuild");
  _79 = String_new("--clean");
  _80 = String_new("--build-dir");
  _81 = String_new("--compile-commands");
  _82 = String_new("--save-temps");
  _83 = String_new("--save-temps[=<dir>]");
  _84 = String_new("--sha256");
  _85 = String_new("--index");
  _86 = String_new("--force");
  _87 = String_new("--prefix");
  _88 = String_new("-I");
  _89 = String_new("--x-include-dir");
  _90 = String_new("--c-include-dir");
  _91 = String_new("--c-system-dir");
  _92 = String_new("--package-dir");
  _93 = String_new("--no-cpp");
  _94 = String_new("--live-symbols");
  _95 = String_new("--cpp-symbols");
  _96 = String_new("--cc");
  _97 = String_new("--ar");
  _98 = String_new("-O");
  _99 = String_new("-O0, -O1, -O2, -O3, -Os");
  _100 = String_new("-g");
  _101 = String_new("-D");
  _102 = String_new("-U");
  _103 = String_new("-Xcc");
  _104 = String_new("-L");
  _105 = String_new("-l");
  _106 = String_new("--rpath");
  _107 = String_new("-Wl,");
  _108 = String_new("-Wl,<arg>[,<arg>...]");
  _109 = String_new("-pthread");
  _110 = String_new("-framework");
  _111 = String_new("-Xlinker");
  _112 = String_new("--dump-tokens");
  _113 = String_new("--dump-cpp");
...
static void _print_option(const CliOption * option){
  ...
  _print_help_row(label, option -> description, String_startswith(label, _42) ? 6 : 2);
}
...
static void _print_help(Symbol command){
  switch(command){
    ...
    break;
    case 535328 : puts(_23);
    break;
    default: x2c_driver_error(String_join(NULL, cons(String_var(_24), cons(String_var(Symbol_str(command)), cons(String_var(_25), NULL)))));
  }
...
static CliOption * _take_option(Array args, int * index, int mask, String * spelling, String * value, int * attached){
  ...
  if(equals > 2 && ! option -> value) x2c_driver_error(String_join(NULL, cons(String_var(_26), cons(String_var(arg), cons(String_var(_25), NULL)))));
  ...
  * value = equals > 2 ? joined : suffix;
  if(option -> value && ! String_truth(* value) && equals <= 2){
    if(++ * index == Array_len(args)) x2c_driver_error(String_join(NULL, cons(String_var(_27), cons(String_var(arg), cons(String_var(_25), NULL)))));
...
static void _driver_kind(CliRequest request, String value){
  ...
  if(String_equal(value, _28)) request -> kind = 404971770155786;
  else if(String_equal(value, _29)) request -> kind = 1381098885964356;
  else if(String_equal(value, _30)) x2c_driver_error("shared-library is not supported by this compiler");
  else x2c_driver_error(String_join(NULL, cons(String_var(_31), cons(String_var(value), cons(String_var(_25), NULL)))));
}
...
static int _driver_count(String value, int minimum, String noun){
  ...
  if(! String_truth(value) || errno || * end || count < minimum || count > INT_MAX) x2c_driver_error(String_join(NULL, cons(String_var(_32), cons(String_var(noun), cons(String_var(_33), cons(String_var(value), cons(String_var(_25), NULL)))))));
  return(int) count;
}
...
static void _apply_option(CliRequest c, CliOption * option, String spelling, String value, int attached, Array x_paths, Array cpp_args, Array cc_args, Array ld_args){
  switch(option -> id){
    ...
    if(String_equal(value, _34)) c -> color_mode = 109854;
    else if(String_equal(value, _35)) c -> color_mode = 93785702;
    else if(String_equal(value, _36)) c -> color_mode = 29733220;
    else x2c_driver_error(String_join(NULL, cons(String_var(_37), cons(String_var(value), cons(String_var(_25), NULL)))));
    break;
    ...
    if(c -> command != 45220543335690) _push_pair(cc_args, _88, value);
    break;
    ...
    break;
    case 49350 : if(cli_dependency_pass_through(value)) x2c_driver_error(String_join(NULL, cons(String_var(_38), cons(String_var(value), cons(String_var(_25), NULL)))));
    ...
    break;
    case 38800656 : Array_push(ld_args, String_var(String_join(NULL, cons(String_var(_39), cons(String_var(value), NULL)))));
    break;
...
CliRequest cli_package_options(String path, String package){
  ...
  x2c_cleanup_push(&_x2c_defer_record_0);
  {
    ...
    for(int i = 0;  i < Array_len(words);  i ++){
      ...
      if(! option) x2c_driver_error(String_join(NULL, cons(String_var(_40), cons(String_var(argument), cons(String_var(_25), NULL)))));
      switch(option -> id){
        ...
        default: x2c_driver_error(String_join(NULL, cons(String_var(_40), cons(String_var(argument), cons(String_var(_25), NULL)))));
      }
...
static void _one_dash_removed(String arg){
  String attached;
  if(String_len(arg) > 2 && String_getindex(arg, 0) == '-' && String_getindex(arg, 1) != '-' && _find_option(String_join(NULL, cons(String_var(_41), cons(String_var(arg), NULL))), CLI_TRANSLATE, & attached)){
...
static CliRequest _parse_command(Array args, CliCommand * command){
  ...
  for(int i = 1;  i < Array_len(args);  i ++){
    ...
    if(mask != CLI_BOOTSTRAP && ! operands && dashed && String_equal(arg, _42)){
      operands = 1;
      continue;
    }
    ...
    if(! option){
      ...
      x2c_driver_error(String_join(NULL, cons(String_var(_45), cons(String_var(arg), cons(String_var(_25), NULL)))));
    }
  ...
  if((mask == CLI_INSTALL || mask == CLI_REMOVE || mask == CLI_NEW) &&(! List_truth(request -> inputs) || List_truth(List_cdr(request -> inputs)))) x2c_driver_error(String_join(NULL, cons(String_var(Symbol_str(name)), cons(String_var(_46), NULL))));
...
CliRequest cli_parse(int argc, char * * argv){
  ...
  x2c_cleanup_push(&_x2c_defer_record_1);
  {
    ...
    if(String_equal(first, _47) || String_equal(first, _48)){
      _print_help(0);
      exit(0);
    }
    ...
    if(String_equal(first, _51)){
      ...
      if(String_equal(name, _51) || String_equal(name, _47) || String_equal(name, _48)) _print_help(535328);
      else if(asked) _print_help(asked -> name);
      else x2c_driver_error(String_join(NULL, cons(String_var(_24), cons(String_var(name), cons(String_var(_25), NULL)))));
      exit(0);
    }
    ...
    if(String_equal(first, _44)) _removed_output();
    String attached;
    if(String_len(first) > 2 && String_getindex(first, 0) == '-' && String_getindex(first, 1) != '-' && _find_option(String_join(NULL, cons(String_var(_41), cons(String_var(first), NULL))), CLI_TRANSLATE, & attached)){
      fprintf(stderr, "x2c: error: one-dash long option '%s' was removed\n", first);
      fprintf(stderr, "note: use 'x2c translate --%s ...'\n", first + 1);
      exit(2);
    }
    if(String_getindex(first, 0) != '-') _expected_command(first);
    x2c_driver_error(String_join(NULL, cons(String_var(_52), cons(String_var(first), cons(String_var(_25), NULL)))));
  }
  x2c_cleanup_leave(& _x2c_defer_record_1);
