...
static char * _manifest(String * identity){
  ...
  ErrorHandler volatile _x2c_error_handler_0 = x2c_error_catch_site_push(&_x2c_exception_frame_0, &_x2c_catch_site_0, _x2c_catch_patterns_0);  x2c_exception_push(& _x2c_exception_frame_0);  if (!sigsetjmp(_x2c_exception_frame_0.env, 0)) content = File_string_close(input);  else {x2c_exception_landed(& _x2c_exception_frame_0); {
...
static void _extract(Bootstrap payload, char * manifest){
  ...
  while(line){
    ...
    String parent = Path_dirname(target);
