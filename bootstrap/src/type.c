...
_x2c_initializer_choice_2B6ED6EF_2((_x2c_func_handle_0 = Func_new(_x2c_func_adapt_0, _313)))
...
String Type_var_numeric_extractor(Type type){
  if(! _init_guard_) _file_init_();  if(Type_is_enum(type)) return _314;  List row = _scalar_row(type);  return List_truth(row) ? Var_string(List_cadr(row)) : NULL;
}
