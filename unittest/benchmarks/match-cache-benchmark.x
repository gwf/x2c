/*  match-cache-benchmark.x -- Match cache-boundary acceptance benchmark
    ...
    Measures the production cached-adapter boundary against the uncached
    per-call route a dynamic pattern pays without it: the frozen six-family
    corpus, the prepared / hit / cold / negative lanes, and the
    sublis-document-migration product workload. `--check` proves oracle parity and the zero-losing-span contract
...
/* The uncached route a dynamic pattern pays without the cache: one
...
static uint64_t _time_percall_match(
...
static void _time_family(
  ...
  if (candidate_first) {
    candidate_ns = _time_adapter_match(cache, input, pattern,
                                       iterations,
                                       &candidate_receipt);
    reference_ns = _time_percall_match(input, pattern, iterations,
                                       &reference_receipt);
  }
  else {
    reference_ns = _time_percall_match(input, pattern, iterations,
  ...
  _check(reference_receipt ==
         _expected_percall_match(input, pattern, iterations),
         "reference-receipt");
...
static void _time_mode(int sample) {
  ...
  uint64_t reference_ns = _time_percall_match(status_input,
                                              status_pattern,
                                              iterations,
                                              &reference_receipt);
