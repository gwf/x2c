...
int public_value(int value) => value + 1;
...
static double converted(int value) => value;

static int comma_effect;

static int conditional_comma(int value) =>
  (comma_effect += value < 0 ? -value : value), 3;

static Counter native_compound(int value) =>
  (Counter) { .value = value };

static Map x2c_compound(int value) => %{value: $value};

static String interpolated(int value) => %"value=$value";
...
static int macro_value(int value) => $plus_one(value);

$define_generated(generated_value);

$forward(static int forwarded(int value) => value + 10;);

$identity()
static int decorated(int value) => value + 4;

static int implementation(int value) => value + 1;
...
int declared(int value) => value + object_value;
