...
| C03 | One native function-signature projection | Done 2026-09-24 (4da723ee, then this follow-up): stored signatures retain aliases; Lisp bindings, native meta rows, and ordinary adapters all call `Compiler.func_signature`; native meta matching resolves aliases on both sides, so `Size`, `unsigned long`, and `size_t` match. Lisp-visible spellings keep aliases (`("Byte")`, `("Size")`, `("size_t")`, `(& "Size")`, `("Widget")`), keep `const` (`(* const char)`), and use C scalar names (`(unsigned)`, not `(uint)`), including in `bad-arity`/`bad-types` data and lifetime notes | Stored signatures retain aliases; native checks resolve types where needed |
...
| C12 | One owner for typed Match guard interpretation | Declined by Gary 2026-09-24: removes ~10 compiler lines, open design, Match hot path; the alias difference only skips a specialization | Compiler consumes normalized predicate facts instead of decoding guard sugar independently |
