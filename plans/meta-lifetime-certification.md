...
| `Var.token` | borrowed from the token stream | wrap | none |
...
so its `meta` mark comes from that conversion.
...
3. **Mark the nine memory-only methods `meta`.** This touches
   `lib/split.x`, `lib/common.x`, `lib/dispatch.x`, `lib/autodiff.x`,
   `lib/tokenizer.x` and the `Var(Job)` protocol. It adds new native
   targets, so it takes two rounds:
   - round one adds the marks and refreshes `bootstrap/`;
...
4. **`List.job` in meta.** This also takes two rounds.
   - Add one adapter row in `lib/lisp.x` that pushes `_lowered_owner()`
     around `Job.new`, and add the `meta` mark.
